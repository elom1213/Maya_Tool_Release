...
# A00200_CSV_tool (ARKit Facial Import) - 셸프 버튼 설치 + 드래그&드롭 진입점
#
# 이 파일을 Maya 뷰포트로 드래그&드롭하면 현재 셸프에 버튼이 설치된다.
# 설치된 버튼은 tools.A00200_CSV_tool.run(True) 를 호출한다.
...
TOOL_ROOT = os.path.dirname(__file__)                       # .../tools/A00200_CSV_tool
...
def install_shelf_button():
    ...
    for btn in shelf_buttons:
        ...
        if "A00200_CSV_tool.run(True)" in str(cmd):
