...
# A00030_quickTool - 셸프 버튼 설치 + 드래그&드롭 진입점
#
# 이 파일을 Maya 뷰포트로 드래그&드롭하면 현재 셸프에 버튼이 설치된다.
# 설치된 버튼은 tools.A00030_quickTool.run(True) 를 호출한다.
...
TOOL_ROOT = os.path.dirname(__file__)                       # .../tools/A00030_quickTool
...
TOOL_LABEL = "QuickTool"

_ICON_PATH = os.path.join(TOOL_ROOT, "icon", "A00030_quickTool.png")
ICON_NAME = _ICON_PATH if os.path.exists(_ICON_PATH) else "pythonFamily.png"

# 셸프 버튼이 실행할 명령.
# ROOT 는 JUN_All 경로여야 `import tools.A00030_quickTool` 가 동작한다.
SHELF_COMMAND = r'''
import sys
...
import tools.A00030_quickTool as A00030_quickTool

A00030_quickTool.run(True)
'''.format(
