...
처음 받은 직후 **`update.bat`** 을 한 번 실행하세요. 내부적으로 `setup_app_dir.py` 가 실행되어
Maya `scripts/userSetup.py` 에 이 저장소의 `tools/` 경로가 등록됩니다.
...
클론으로 받았다면, 저장소 루트의 **`update.bat`** 을 더블클릭하세요. 두 단계를 수행합니다.
